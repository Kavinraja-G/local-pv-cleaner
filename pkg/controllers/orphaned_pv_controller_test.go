	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			r := &PVCleanupController{
				...
				EnableNodeWatchers:      tt.args.EnableNodeWatchers,
				EnablePeriodicCleanup:   tt.args.EnablePeriodicCleanup,
				NodeSelectorKeys:        tt.args.NodeSelectorKeys,
				PeriodicCleanupInterval: tt.args.PeriodicCleanupInterval,
			}
			if err := r.cleanupOrphanedPVs(ctx); (err != nil) != tt.wantErr {
